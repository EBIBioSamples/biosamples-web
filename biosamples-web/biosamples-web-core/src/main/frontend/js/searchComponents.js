        ...
        function buildBadges(obj) {
            ...
            crtNames.forEach(name => {
                ...
                badges[badgeKey] = obj[name].map(el => {
                    ...
                    try {
                        ...
                        console.error('Error trying to parse badgeValue:', el);
                    }
                    return val;
                });
            });
            ...
            if (extRefs) {
                try {
                    ...
                    console.error('Unable to render badges for external reference', err);
            ...
            saveHistoryState: function(){
                ...
                if ( !_.isEmpty( this.currentQueryParams ) ) {
                    if ( _.isEqual( this.currentQueryParams, this.previousQueryParams ) ) {
                        ...
                        History.replaceState(this.currentQueryParams, null, '?' + utilities.toQueryString(this.currentQueryParams));
                    } else {
                        vueConsole.debug('Push new history state','History');
                        this.previousQueryParams = this.currentQueryParams;
                        History.pushState(this.currentQueryParams, null, '?' + utilities.toQueryString(this.currentQueryParams));
                    }
                }
            }
        }
