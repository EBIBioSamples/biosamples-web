    ...
    */
    ...
    for (var i = 0; i < results.data.facet_counts.facet_fields.content_type.length; i++) {
      if (typeof results.data.facet_counts.facet_fields.content_type[i] !== "string") {
        if (maxOccurence1 < results.data.facet_counts.facet_fields.content_type[i]) maxOccurence1 = results.data.facet_counts.facet_fields.content_type[i];
      }
    }
    var maxOccurence2 = 0;
    for (var i = 0; i < results.data.facet_counts.facet_fields.organ_crt.length; i++) {
      if (typeof results.data.facet_counts.facet_fields.organ_crt[i] !== "string") {
        if (maxOccurence2 < results.data.facet_counts.facet_fields.organ_crt[i]) maxOccurence2 = results.data.facet_counts.facet_fields.organ_crt[i];
      }
    }
    ...
    // domain is input data, range is output to put the data to
    .domain(dataBar2.map(function (d) {
      return d.content;
    ...
    ;
    var xAxis2 = d3.svg.axis().scale(scale2x).orient("bottom");
    var yAxis2 = d3.svg.axis().scale(scale2y).orient("left");
    ;
    ...
    barChart1.selectAll(".bar").data(dataBar1).enter().append("rect").attr("class", "bar").attr("id", function (d) {
      return d.content;
    })
    // space is 5
    .attr("width", widthRectangle1).attr("x", function (d) {
      return d.x;
    }).attr("y", function (d) {
    ...
    for (var i = 0; i < dataBar2.length; i++) {
      ...
      var yHere = height - margin.bottom;
      d3.select("#resultsViz2").append("text").attr("x", function () {
    ...
    if (document.getElementById("vizSpotRelations") === null) {
      svg = d3.select(".container").insert("svg", ":first-child").attr("width", "60%").attr("height", heightD3).attr("id", "vizSpotRelations").style("stroke", "green").style("stroke-width", 1).style("border", "solid").style("overflow", "scroll").style("border-color", "#5D8C83").style("border-radius", "10px").call(d3.behavior.zoom().on("zoom", function (d) {
        svg.attr("transform", "translate(" + d3.event.translate + ")" + " scale(" + d3.event.scale + ")");
      })).append("g");
    } else {
      d3.select("#vizSpotRelations").remove();
      document.getElementById("infoVizRelations").style.visibility = "hidden";
      svg = d3.select(".container").insert("svg", ":first-child").attr("width", "60%").attr("height", heightD3).attr("id", "vizSpotRelations").style("stroke", "black").style("stroke-width", 1).style("border", "solid").style("overflow", "scroll").style("border-color", "#5D8C83").style("border-radius", "10px").call(d3.behavior.zoom().on("zoom", function (d) {
