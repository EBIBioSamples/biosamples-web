...
module.exports = function () {
    ...
    return {
        ...
        getInstance: function getInstance() {
            var customVariables = arguments.length <= 0 || arguments[0] === undefined ? {} : arguments[0];
...
},{"./facetlist.template.html":34}],34:[function(require,module,exports){
...
if (module.hot) {(function () {  module.hot.accept()
  ...
  var id = "/Applications/MAMP/htdocs/clonedAfterChanges/biosamples-web/src/main/frontend/js/components/itemsDropdown/ItemsDropdown.vue"
  if (!module.hot.data) {
...
},{"underscore":3,"vue":29,"vue-hot-reload-api":4}],36:[function(require,module,exports){
'use strict';
...
},{"lodash":1}],44:[function(require,module,exports){
"use strict";
...
(function (window) {
  ...
  var vm = new Vue({
    ...
    computed: {
      queryTermPresent: function queryTermPresent() {
        return !_.isEmpty(this.queryTerm);
      },
      queryHasResults: function queryHasResults() {
    ...
    methods: {
      ...
      querySamplesUsingFuzzy: function querySamplesUsingFuzzy(e) {
        if (e !== undefined) {
          e.preventDefault();
        }
        this.useFuzzy = true;
      ...
      querySamples: function querySamples(e) {
        ...
        this.$http.get(apiUrl, queryParams, ajaxOptions).then(function (results) {
      ...
      consumeResults: function consumeResults(results) {
        ...
        _.forEach(dynamicFacetsKey, function (key) {
          var readableKey = key.replace('_crt_ft', '');
      ...
      serializeFilterQuery: function serializeFilterQuery() {
        var filterArray = [];
        _.each(this.filterQuery, function (value, key) {
          if (!_.isNil(value)) {
      ...
      deserializeFilterQuery: function deserializeFilterQuery(serializedQuery) {
        ...
        _(serializedQuery).forEach(function (value) {
          ...
          var _value$split = value.split("Filter|");

          var _value$split2 = _slicedToArray(_value$split, 2);

          var filterKey = _value$split2[0];
          var filterValue = _value$split2[1];
       ...
       */
      readLocationSearchAndQuerySamples: function readLocationSearchAndQuerySamples() {
...
function doD3Stuff(results, apiUrl) {
  ...
  if (typeof results !== 'undefined') {
    var draw = function draw(svg, nodeData) {
      ...
      var force = d3.layout.force().gravity(.08).distance(50).charge(-100).size([width, height]);
      ...
      node.append("circle").on("mousedown", function (d) {}).on("mouseout", function (d) {}).on("mouseover", function (d) {}).attr("r", function (d) {
        return d.radius;
      }).attr("accession", function (d) {
        return d.accession;
      }).attr("id", function (d) {
      ...
      node.append("text").attr("dx", 12).attr("dy", ".35em").attr("id", function (d) {
        ...
        return "[" + d.accession + "]";
      }).attr("font-family", "sans-serif").attr("font-size", "10px").attr("border", "solid").attr("border-radius", "10px").style("border", "solid").style("border-radius", "10px").style("box-shadow", "gray").style("background-color", "#46b4af").attr("fill", "#4D504F").on("mouseover", function (d) {
        d3.selectAll(".node").selectAll("text").style("font-size", "10px");
      });
    ...
    d3.select("#sectionVizResult").on("mouseenter", function () {
      document.getElementById("elementHelp").style.visibility = "visible";
      document.getElementById("elementHelp").innerHTML = "Help " + "<hr/> • Click on a bar or a text to highlight the nodes with these values." + "<br/> • Double click to filter the results according to a facet. ";
    ...
    if (!(Object.keys(numberFacetsUnEmpty).length === 0 && JSON.stringify(obj) === JSON.stringify({}))) {
      ...
      document.getElementById("infoVizRelations").innerHTML = ' <h3>Clicked element information</h3>' + '<div id ="buttons-display"></div>' + '<div id="textData"> <p> Click on an element of the diagram to display its information </p> </div>';
      ...
      for (var u in numberFacetsUnEmpty) {
        if (numberFacetsUnEmpty[u] > 0) {
          strResults += '<td>  <div id="rezFacets' + cpt + '" height=' + document.getElementById("areaNodeLink").getBoundingClientRect().height / 3 + ' overflow=scroll; overflow-x=visible  > </div>   </td>';
        }
        cpt++;
      }
      strResults += '</tr> </table>';
    ...
    for (var u in numberFacetsUnEmpty) {
      if (numberFacetsUnEmpty[u] > 0) {
        ...
        barCharts.push(d3.select(idToSelect).insert("svg", ":first-child").attr("width", function () {
        ...
        .attr("id", "resultsViz" + cpt + "").attr("class", "bar").style("stroke", "black").style("stroke-width", 1).style("border", "solid").style("border-color", "#5D8C83").style("border-radius", "10px"));
      } else {
    ...
    for (var u in numberFacetsUnEmpty) {
      dataBars.push([]);
      scalesX.push(d3.scale.ordinal().domain(dataBars[cpt].map(function (d) {
        console.log("d.content : " + d.content);return d.content;
    ...
    var cpt = 0;
    for (var u in numberFacetsUnEmpty) {
      xAxises.push(d3.svg.axis().scale(scalesX[cpt]).orient("bottom"));
    ...
    for (var h = 0; h < dataBars.length; h++) {
      for (var i = 0; i < dataBars[h].length; i++) {
        ...
        d3.select(idToSelect).append("text").attr("class", "text-d3").attr("content", function (d) {
          ...
          return 0;
        }).attr("dy", ".71em").attr("opacity", "1").on("mouseover", function (d) {
          document.getElementById("elementHelp").style.visibility = "visible";
          ...
          document.getElementById("elementHelp").innerHTML = "Help" + "<hr/> " + facet + " <hr/> " + content + " : " + occurence;
          ...
          d3.select("#vizSpotRelations").selectAll(".node").select("circle").style("stroke", function (d) {
            ...
            for (var u in d.responseDoc) {
              ...
              if (stringResponse.indexOf(content) > -1) {
                ...
                if (!gotHighlighted) {
                  cptHighlighted++;gotHighlighted = true;
                }
              }
            }
          });
        ...
        }).attr("style", "stroke:red;writing-mode: tb; glyph-orientation-vertical: 90").text(function () {
    ...
    for (var h = 0; h < barCharts.length; h++) {
      barCharts[h].selectAll(".bar-d3").data(dataBars[h]).enter().append("rect").attr("class", "bar-d3").attr("id", function (d) {
      ...
      // space is 5
      .attr("width", widthRectangles[h]).attr("fill", "#46b4af").on("mouseover", function (d, i) {
        document.getElementById("elementHelp").style.visibility = "visible";
        document.getElementById("elementHelp").innerHTML = "Help" + "<hr/>" + d.facet + " <hr/> " + d.content + ", " + d.occurence;
        var modifiedContent = changeSpecialCharacters(d.content);
    ...
    var svg;
    svg = d3.select("#vizNodeLink").insert("svg").attr("width", "100%").attr("height", heightD3).attr("id", "vizSpotRelations").style("stroke", "black").style("stroke-width", 1).style("border", "solid")
    //.style("overflow","scroll")
    .style("border-color", "#5D8C83").style("border-radius", "4px").on("mouseenter", function () {
      document.getElementById("elementHelp").style.visibility = "visible";
      d3.select("#elementHelp").html("Help <hr/> Hover over a node to make it bigger. <br/> Click on a node to display its information.");
    }).on("mouseleave", function () {
      document.getElementById("elementHelp").style.visibility = "hidden";
...
},{"./components/BioSample.js":30,"./components/Store.js":31,"./components/badge/Badge.js":32,"./components/facetList/FacetList.js":33,"./components/itemsDropdown/ItemsDropdown.vue":35,"./components/pagination/Pagination.js":36,"./components/productsList/ProductsList.js":40,"./filters/excerptFilter.js":42,"./filters/startCaseFilter.js":43,"./utilities/_mixins.js":45,"lodash":1,"vue":29,"vue-resource":18}],45:[function(require,module,exports){
'use strict';
